...
export function BookingsChart({ data }: BookingsChartProps) {
  ...
  const points = useMemo(() => {
    return (data || [])
      ...
      .map((p) => ({ date: p.date.slice(0, 10), count: Number.isFinite(p.count) ? p.count : 0 }))
  ...
  const chart = useMemo(() => {
    ...
    if (points.length === 0) {
      return { width, height, padding, maxY: 0, curvePath: '', areaPath: '', dots: [] as Array<{ x: number; y: number; date: string; count: number }>, xTicks: [] as Array<{ x: number; label: string }>, yTicks: [] as Array<{ y: number; label: string }> }
    }
    ...
    const dots = points.map((p, i) => ({ x: xForIndex(i), y: yForValue(p.count), date: p.date, count: p.count }))
    ...
    return { width, height, padding, maxY, curvePath, areaPath, dots, xTicks, yTicks }
  }, [points])
  ...
  return (
    <div>
      <div className="mb-4">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-[#8a7a92]">Réservations confirmées</p>
        <h3 className="mt-1 text-base font-bold text-[#2A1F2D]">Évolution des réservations</h3>
      </div>
      ...
      {isEmpty ? (
        ...
        <div className="relative" ref={containerRef}>
          {hoverIndex !== null && chart.dots[hoverIndex] && (
            <div
              className="pointer-events-none absolute z-10 rounded-[14px] border border-primary/15 bg-white px-3.5 py-2.5 shadow-[0_4px_16px_rgba(20,0,50,0.12)]"
              style={{
          ...
          <svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="w-full h-auto" onMouseLeave={() => setHoverIndex(null)} onMouseMove={handleMouseMove}>
            <defs>
              <linearGradient id="bookingsGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="rgba(200,109,215,0.25)" />
                <stop offset="100%" stopColor="rgba(200,109,215,0.02)" />
              </linearGradient>
            </defs>
            ...
            {/* Gradient area */}
            <path d={chart.areaPath} fill="url(#bookingsGrad)" />

            {/* Smooth curve */}
            <path d={chart.curvePath} fill="none" stroke="rgba(200,109,215,1)" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />
            ...
            {chart.dots.map((d, i) => {
              ...
              return (
                <g key={i}>
                  {active && <circle cx={d.x} cy={d.y} r={14} fill="rgba(200,109,215,0.1)" />}
                  <circle cx={d.x} cy={d.y} r={active ? 6 : 4} fill={active ? '#9C44AF' : '#C86DD7'} stroke="white" strokeWidth={2.5} />
                </g>
              )
            })}

            {hoverIndex !== null && chart.dots[hoverIndex] && (
              <line x1={chart.dots[hoverIndex].x} x2={chart.dots[hoverIndex].x} y1={chart.padding.top} y2={chart.height - chart.padding.bottom} stroke="rgba(200,109,215,0.12)" strokeWidth={1.5} strokeDasharray="5 4" />
            )}
          </svg>
        </div>
      )}
